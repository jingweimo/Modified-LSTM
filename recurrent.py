...
class LSTM1(Recurrent):
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            if self.implementation == 0:
                ...
                #LSTM1: No input signals for input, forget and output gates
                x_i = 0*K.dot(inputs * dp_mask[0], self.kernel_i) + self.bias_i
                x_f = 0*K.dot(inputs * dp_mask[1], self.kernel_f) + self.bias_f
                x_c = K.dot(inputs * dp_mask[2], self.kernel_c) + self.bias_c
                x_o = 0*K.dot(inputs * dp_mask[3], self.kernel_o) + self.bias_o
            else:
                raise ValueError('Unknown `implementation` mode.')
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(LSTM1, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            if self.implementation == 0:
                ...
                x_i = 0*K.dot(inputs * dp_mask[0], self.kernel_i) + 0*self.bias_i
                x_f = 0*K.dot(inputs * dp_mask[1], self.kernel_f) + 0*self.bias_f
                x_c = K.dot(inputs * dp_mask[2], self.kernel_c) + self.bias_c
                x_o = 0*K.dot(inputs * dp_mask[3], self.kernel_o) + 0*self.bias_o
            else:
                raise ValueError('Unknown `implementation` mode.')
            #No input and bias for input, forget and output gates
            i = self.recurrent_activation(x_i + K.dot(h_tm1 * rec_dp_mask[0],
                                                      self.recurrent_kernel_i))
            f = self.recurrent_activation(x_f + K.dot(h_tm1 * rec_dp_mask[1],
                                                      self.recurrent_kernel_f))
...
class LSTM3(Recurrent):
    ...
    def __init__(self, units,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(LSTM3, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            if self.implementation == 0:
                ...
                #Not input signals for input, forget and output gates
                x_i = 0*K.dot(inputs * dp_mask[0], self.kernel_i) + self.bias_i
                x_f = 0*K.dot(inputs * dp_mask[1], self.kernel_f) + self.bias_f
                x_c = K.dot(inputs * dp_mask[2], self.kernel_c) + self.bias_c
                x_o = 0*K.dot(inputs * dp_mask[3], self.kernel_o) + self.bias_o
            ...
            i = self.recurrent_activation(x_i + 0*K.dot(h_tm1 * rec_dp_mask[0],
                                                      self.recurrent_kernel_i))
            f = self.recurrent_activation(x_f + 0*K.dot(h_tm1 * rec_dp_mask[1],
                                                      self.recurrent_kernel_f))
            c = f * c_tm1 + i * self.activation(x_c + K.dot(h_tm1 * rec_dp_mask[2],
                                                            self.recurrent_kernel_c))
            o = self.recurrent_activation(x_o + 0*K.dot(h_tm1 * rec_dp_mask[3],
                                                      self.recurrent_kernel_o))
        h = o * self.activation(c)
        if 0 < self.dropout + self.recurrent_dropout:
            h._uses_learning_phase = True
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(LSTM3, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))